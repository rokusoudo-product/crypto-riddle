...
> **UI Issue #102** で行う。**さらに #108（docs のみ）で、会話フレームのレイアウトを「3枠の対策室レイアウト」から
> 「導入・探索・解決すべて共通の左右2枠の入れ替わり方式」に改訂した（S1 実装台本レビュー第1回・2026-09-13 代表）。
> このレイアウト変更はスキーマ（本節・zod とも）に影響しない。UI 実装は新設の **UI Issue #110** で行う（NPC名札・
> explanations 話者表示・表情フォールバックの実装は引き続き #102 のスコープ）。
