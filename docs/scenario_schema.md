...
- **`portrait` 省略時の表示**: `portrait` を持たないホットスポットは、縦長の画面でも `landscape` の座標を
  そのまま使う（縦長の画面でも背景の箱を **16:9 のまま**＝画面幅にフィットさせて表示し、立ち絵の比率も
  横用の値を使う方針。`DESIGN.md`「探索シーン」節「背景の箱」「背景」参照）。この対応づけは UI 側の表示
  ロジック（#124）が担い、スキーマ側に分岐フィールドは持たせない。
