...
updated: 2026-09-15（§2.8 追加: ゲーム内時刻を表す省略可能な `game_time`（`HH:MM`・24時間表記）を
  ...
  （秘書レビュー・代表決定2026-09-14・#124。#125の記述を改める）。#123 で S1 の全ホットスポットに
  `portrait` 座標を投入・導入の背景を `bg-hq-taskforce` へ差し替え、§2.7 の「現時点では全シーンが
  該当」の記述を更新）
---
...
> （台本 v2.2 への移植）のみ**に変更されている。NPC 名札・`explanations` 話者表示・表情フォールバックは
> **#102（PR #106）で実装済み**。会話フレームの左右2枠の入れ替わり方式の UI 実装は **#110**。
...
- **背景アセット**は image_agent 自作（横16:9／縦9:16（#119）・アニメ調で立ち絵と統一。DESIGN.md「探索シーン」「アセット」節）。`background` はアセットIDで参照し、YAML にパスを直書きしない。
- `scenes`/`hotspots`/`object_type`/座標系は **T037（0.4.0）で確定済**。`collect` の `line`/`speaker` は **T043（0.5.0）で確定**。`goto`/`door`/`prompt` は **T046（0.6.0）で確定**。**`hotspots[].position` を横・縦の組にする改訂は 0.8.0（#119 で仕様確定・#120 で実装予定）。詳細は §2.7**。
...
理由: 会話フレームの描画枠は**左右2枠の入れ替わり方式**（#108。`DESIGN.md`「会話フレーム」節）であり、
枠自体は3人目（小鳥遊）が話しても技術的には描画できる。それでも探索・解決への登場を型（スキーマ）で拒否するのは、
小鳥遊が**庶務担当のため現場（客先）には同行しない**という人物設定（`docs/characters.md`／`specs/001-mvp/spec.md`）
を、UI の描画可否ではなくデータの構造として守るため。登場は**①導入（対策室での事件発生の一報）と⑦結果（クリア後
のねぎらい）のみ**に限定する。
...
### 2.7 スキーマ 0.8.0（ホットスポット座標を横・縦の組にする改訂・#119 で仕様確定／#120 で zod 実装予定）

> **背景**: #119（本 Issue・docs のみ）で、背景を**画面の向き**（横長の画面＝16:9／縦長の画面＝9:16）で
> 切り替える方式に改訂した（`DESIGN.md`「探索シーン」節「背景の箱」）。ホットスポットの相対座標は
> 背景1枚に対して決まるため、横・縦で背景画像が変わる場合は**座標も横・縦で別に持つ**必要がある。
> zod 改訂は **core Issue #120**、S1 の縦座標データ投入は **data Issue #123**。UI 実装は **#124**。

`scenes[].hotspots[].position` を、単一の `[x, y]` 配列から**横・縦の組を持つオブジェクト**へ変更する。
...
- **`position` はオブジェクト必須**（`z.object({ landscape: <座標>, portrait: <座標>.optional() })`）。
  旧 `[x, y]` 配列形式（0.4.0〜0.7.0）は**廃止**し、既存4マップ（S1/S2/S3/SL）の全ホットスポットは
  現在の座標値をそのまま `landscape` へ機械移植する（値は変更しない。**#120 で実施**）。
- **`landscape` は必須・`portrait` は当面省略可**（縦の背景がまだ無いシーン向け）。**#123 で S1（執務室・
  サーバ室）の全ホットスポットに `portrait` を投入済み**（縦の背景 `bg-s1-office-portrait`/
  `bg-s1-server-portrait` があるため下記「縦の背景があるシーンでの `portrait` 座標の必須化」の対象）。
  S2/S3/SL は縦の背景がまだ無いため引き続き省略可のまま。**4マップすべてに縦の背景・縦座標が揃った時点で
  `portrait` を必須に変更する**（将来の後続 Issue で判断。本節はその前段の「当面は省略可」の状態を
  定義する）。
- **`portrait` 省略時の表示（縦長の画面は常に9:16に改訂・代表決定2026-09-14・#124。#125の記述を改める）**:
  縦長の画面は、縦の背景アセットの有無に関わらず**常に 9:16 の箱**にする（旧「縦の背景が無いシーンは
  縦長の画面でも16:9の箱のまま」は撤回）。`portrait` を持たないホットスポットは、9:16 の箱の**上部**に
  表示する横長画像（`landscape` の背景、幅いっぱい・`contain`・上寄せ）に対する相対座標として
  `landscape` の座標をそのまま使う（座標変換自体はUI側=`resolveHotspotBoxPosition`が行う。
  `DESIGN.md`「探索シーン」節「背景の箱」「背景」参照）。この対応づけは UI 側の表示ロジック（#124）が
  担い、スキーマ側に分岐フィールドは持たせない。
...
- **縦の背景があるシーンでの `portrait` 座標の必須化（食い違い検出）**: 縦の背景アセットはシーン単位で
  有無が決まる（上記）。**縦の背景があるシーン（対応表に `-portrait` の解決先を持つシーン）で `portrait`
  座標が無いホットスポット**は、`landscape`/`portrait` の食い違いとして**許さない**（#124 の食い違い
  検出テストで検出する）。
...
- 条文番号・報告期限が**改正で変わりうる**ため、シナリオのセリフやカード本文に直接埋め込まない
  (docs/characters.md「制作上の注意」も同趣旨)。
