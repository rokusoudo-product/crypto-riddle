...
- [ ] **T047** 探索の会話オーバーレイ化（2状態）＋一覧トグル（T018''''' 代表モック・依存: T044/T046-ui-data）
  ...
  - 完了条件: 探索状態で立ち絵/ウィンドウが出ずホットスポット探索できる／会話状態で重ね表示＆閉じるで復帰／一覧トグルでキーボード完遂／Vitest・Playwright e2e・build 通過。既存4マップを壊さない。
...
- [ ] **T048** 探索オーバーレイの微調整（T018'''''' 代表FB・依存: T047）
  ...
  - 別途（IMAGE_WORKFLOW）: **S1 執務室背景を 中野＋経理部長 の2名入りに再生成**し、経理部長ホットスポットを人物位置へ調整（現状は空きキャビネット付近で分かりづらい＝キャラリスト「要検討」の経理部長外見もここで確定）。代表承認後に実施。

**チェックポイント③'''''''**: T048＋S1経理部長 反映後、代表が確認。
...
- [ ] **T060-ui** 向き別背景の全面表示と立ち絵・会話ウィンドウの重ね直し（#124・#118/#110 の後継・依存: T055-docs, T056-core）
  ...
  - `portrait` を持たないホットスポットは縦長の画面でも `landscape` 座標で表示するフォールバックを実装。**S1 の縦背景（#121・T057-img）がまだの場合は横の背景で実装を進め、縦の背景が入った時点で見た目を確認する**（#124 本文の前提どおり、S1 縦座標データ・導入背景差し替え〔T059-data・#123〕の完了は待たない）。
  - 完了条件: Vitest・Playwright e2e・axe が通る。既存4マップ（S1/S2/S3/SL）を壊さない。話者枠は白で表示される。

**チェックポイント（Phase 4.10 完了後）**: 代表が S1 通しプレイ（横・縦双方の画面）で最終確認する（話者の枠の色は白に確定済み）。
...
- [ ] **T069-img** 探索背景10枚の作り直し（#140〜#143・依存: T061-docs のマージ・代表承認。量産マップは対象外）
  ...
  - 完了条件: 代表が承認した10枚が `assets/backgrounds/` に差し替わり、`DESIGN.md` の該当プロンプト（P-3/P-4/P-5/P-6/P-7/P-8/P-9/P-10/P-13/P-14）が更新されている。S1/S2/S3/SL 各マップの `scenarios/*.yaml` と対応する fixture のホットスポット座標が新背景に合わせて更新され、`npm run build:data`/`test`/`typecheck`/`lint`/`build`/Playwright が成功する。S2/S3/SL は `docs/characters.md` §13.2 に NPC 外見メモが追記されている。
